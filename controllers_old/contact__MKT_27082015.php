...
class Contact extends CI_Controller {
	...
	function requestquotefrm()
	{
		...
		if(isset($tokenmatch[$tokenhidden_key]) && !empty($tokenmatch[$tokenhidden_key]) && $tokenmatch[$tokenhidden_key] == $tokenhidden_value)
		{
			...
			$this->session->unset_userdata('token');
			//if((strlen(strstr($name,$needle))==0) && (strlen(strstr($firstname,$needle))==0)  && (strlen(strstr($email,$needle))==0) && (strlen(strstr($phoneno,$needle))==0) && (strlen(strstr($price,$needle))==0) && (strlen(strstr($comment,$needle))==0))
			//{
...
<div>Une conseillère reviendra vers vous dans les plus brefs délais.</div>
		...
		{
			$url = $_SERVER['HTTP_REFERER'];
			redirect('confirmation-demande-de-devis2');
		}
