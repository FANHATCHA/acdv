...
class Commonlibmodel extends CI_Model{
	...
	function getcatslugtocatname($catslug)
	{
		...
		if(isset($catslug['category_name']) && !empty($catslug['category_name']))
		{
	...
	function getcatslgtoUserid($catslug)
	{
		...
		$catslugs = $query->row_array();
		if(isset($catslugs['user_details_id']) && !empty($catslugs['user_details_id']))
		{
			return $catslugs['user_details_id'];
		}
		else
		{
			return '';
	...
	public function menutreestructure_mobile($menuleve_data){
		$output = '';
		if(isset($menuleve_data) && !empty($menuleve_data))
		{
			foreach($menuleve_data as $menuleve)
			{	$mainmenuclass = '';
				$mainmenuclass = strtolower(trim($this->getmenuURL_mobile($menuleve['id'])));
				$output .='<li>'.$this->getmenuURL_mobile($menuleve['id']).'';
							if(isset($menuleve['children']) && !empty($menuleve['children']))
							{
									$i=0 ;
									$output .='<ul>';
									...
									{		$menuclass = '';
											...
											if($i > 0){ $menuclass = "menu_sub_cols".$i; }
												if(isset($menuleve['id']) && !empty($menuleve['id']) && $menuleve['id'] == '321')
												{
													...
													$output .='<a href="javascript::void();">'.strip_tags($this->getmenuURL_mobile($menuleve2['id'])).'</a>';
													
												}else
												{
													
													$output .=strip_tags($this->getmenuURL_mobile($menuleve2['id']),'<a>');
												...
												if(isset($menuleve2['children']) && !empty($menuleve2['children']))
												{
													...
													foreach($menuleve2['children'] as $menuleve3)
													{
														$output .='<li>'.strip_tags($this->getmenuURL_mobile($menuleve3['id']),'<a>').'</li>';
													}
													$output .='</ul>';
												}
												$output .='</li>';
	...
	function getmenuURL_mobile($id)
	{
		...
		foreach($menudetails as $assmenulist)
		{
			if($assmenulist['menu_type'] == 'custom_link')
			{
				...
				{
					$customlink ='<a href="javascript::void(0);">'.$assmenulist['custom_link_title'].'</a>';
				}
				return $customlink;
			}
			if($assmenulist['menu_type'] == 'cms_page')
			{
				...
				if($assmenulist['clickable'] == 'yes')
				{
					$CMSpagelink = '<a href='.$cmslink.'>'.$cmspage[0]['cms_title'].'</a>';
				}
				else
				{
					$CMSpagelink ='<a href="javascript::void(0);">'.$cmspage[0]['cms_title'].'</a>';
				}
				return $CMSpagelink;
			...
			if($assmenulist['menu_type'] == 'primary_tag')
			{
				...
				{
					$primarytaglink ='<a href="javascript::void(0);">'.$primerytag[0]['tag_name'].'</a>';
				}
				return $primarytaglink;
			}
			if($assmenulist['menu_type'] == 'secondary_tag')
			{
				...
				if($assmenulist['clickable'] == 'yes')
				{
					$secoundrytaglink = '<a href='.$taglink.'>'.$secoundrytag[0]['tag_name'].'</a>';
				}
				else
				{
					$secoundrytaglink ='<a href="javascript::void(0);">'.$secoundrytag[0]['tag_name'].'</a>';
				}
				return $secoundrytaglink;
			}
			if($assmenulist['menu_type'] == 'pro_cat')
			{
				...
				{
					$PROcatlink ='<a href="javascript::void(0);">'.$procatname[0]['category_name'].'</a>';
				}
				return $PROcatlink;
