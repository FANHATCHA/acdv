...
Class Destinationmodel extends CI_Model
{
	...
	function gettotalproductAll($id,$filterdata)
	{
		$query = $this->db->query("select prt.product_id from ".$this->products_rel_table." as prt LEFT JOIN ".$this->products_table." as p ON(prt.product_id = p.id) WHERE status IN('active') AND prt.cat_id = '".$id."' ".$filterdata." ");
	...
	function getdestinationproductdataALL($id,$limit,$offset,$filterdata,$filterdata_order)
	{
		$query = $this->db->query("select pRel.product_id,pRel.cat_id from ".$this->products_rel_table." as pRel JOIN ".$this->products_table." as p ON (pRel.product_id = p.id) WHERE p.status IN('active') AND pRel.cat_id = '".$id."' ".$filterdata."  GROUP BY pRel.product_id  ".$filterdata_order." LIMIT ".$offset.",".$limit."");
