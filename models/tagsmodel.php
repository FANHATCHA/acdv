...
Class Tagsmodel extends CI_Model
{
	...
	function gettagsproductdataALL($slug,$limit,$offset,$filterdata_order)
	{
		$tagid = $this->gettagnametoid($slug);
		$query = $this->db->query('select pt.product_id as id from '.$this->pro_rel_tag_table.' as pt LEFT JOIN '.$this->products_table.' as p ON(p.id = pt.product_id) WHERE pt.tag_id = "'.$tagid.'" AND p.status IN("active") '.$filterdata_order.' LIMIT '.$offset.','.$limit.'');
		return $query->result_array();
	}
	function gettotalproductALL($slug,$total_limit)
	{
