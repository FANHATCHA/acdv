...
<div class="piratical-information">
	...
	<div class="wrapper">
		<div class="info_tabing_web">
			<div id="web_tabing_id">
			<?php	$slug = $this->uri->segment_array();
			...
			if(empty($destinationname)){
				$comnmod = new Commonlibmodel();
				$producturl = end(explode('/',$this->input->server('HTTP_REFERER')));
				$proid = $comnmod->getproslugtoproID($producturl);
				$destionationslug = $comnmod->getproductidtodestinationslug($proid);
				$destinationname = $commod->getcatslugtocatname($destionationslug);
			}
			...
			<div class="proinfo-brd">
				<div class="breadcrumbs">
					<a href="<?php echo $this->config->base_url();?>"><span>Accueil</span></a> > <a href="<?php echo $this->config->base_url().'destination/'.$destionationslug;?>"><span><?php echo ucfirst($destinationname);?></span></a> > <span><?php echo ucfirst($infopagename);?></span>
				</div>
			</div>
				...
				<div class="pro-bottom-info">
					<ul id="product-info-tabs">
						<?php 
							...
							?>
							<li id="tab1_product" <?php echo $activeclass;?>><a href="<?php echo $infopagelinkquand;?>"><?php echo $detailscatslugandname['category_name'];?></a></li>
							<?php 
							}
						?>
					</ul>
...
$( window ).resize(function() {
	if($('.info_tabing_web').css('display') == 'none')
	{
		window['removedHTML'] = $('.info_tabing_web #web_tabing_id').html();
		$('.info_tabing_web #web_tabing_id').remove();
	}
	else
	{
		$('.info_tabing_web').html(window['removedHTML']);
	}
});
</script>